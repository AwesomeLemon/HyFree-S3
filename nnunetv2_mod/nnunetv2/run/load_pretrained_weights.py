...
def load_all_pretrained_weights(network, fname, verbose=False):
    """
    Like the above, but actually load all the weights.
    """
    saved_model = torch.load(fname)
    pretrained_dict = saved_model['network_weights']
    ...
    model_dict = mod.state_dict()
    # verify that all layers have the same shape
    for key, _ in model_dict.items():
        assert key in pretrained_dict, \
            f"Key {key} is missing in the pretrained model weights. The pretrained weights do not seem to be " \
            f"compatible with your network."
    ...
    # model_dict.update(pretrained_dict)
    # model_dict_updated = copy.deepcopy(model_dict)
    ...
    # return pretrained_dict_orig, pretrained_dict_updated, model_dict_updated
