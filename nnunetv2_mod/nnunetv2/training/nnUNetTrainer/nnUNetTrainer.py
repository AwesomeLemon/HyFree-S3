...
from nnunetv2_mod.nnunetv2.training.lr_scheduler.polylr_warmup import PolyLRWarmupScheduler
...
class nnUNetTrainer(object):
    ...
    def configure_optimizers(self):
        ...
        if self.schedule_name == 'poly':
            lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        elif self.schedule_name == 'poly_warmup':
            print('Warmup schedule!')
            lr_scheduler = PolyLRWarmupScheduler(optimizer, self.initial_lr, self.num_epochs)
        else:
    ...
    def get_tr_and_val_datasets(self):
        ...
        # print('AAAAAAAAAA, keeping files open!!!')
    ...
    def get_dataloaders(self):
        ...
        # print(f'{allowed_num_processes=}')
        if allowed_num_processes == 0:
            mt_gen_train = SingleThreadedAugmenter(dl_tr, tr_transforms)
            mt_gen_val = SingleThreadedAugmenter(dl_val, val_transforms)
        else:
            # print(f'!!!Increased num_cached to {allowed_num_processes}')
    ...
    def on_train_start(self):
        ...
        if_eval_initialization = False
        if if_eval_initialization:
            with torch.no_grad():
                ...
                self._best_ema = mean_fg_dice

                self.print_to_log_file(f"Initialization pseudo Dice: {np.round(self._best_ema, decimals=4)}")
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        if_perform_actual_validation = False
        if not if_perform_actual_validation:
            return
